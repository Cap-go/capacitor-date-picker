...
export class DatePickerWeb extends WebPlugin implements DatePickerPlugin {
  private activePicker?: ActivePicker;
  ...
  async present(options: DatePickerOptions = {}): Promise<DatePickerResult> {
    ...
    return new Promise<DatePickerResult>((resolve) => {
      ...
      overlay.querySelector('[data-capgo-picker-body]')?.append(input);
      overlay.querySelector('[data-capgo-picker-actions]')?.append(actions.cancel, actions.done);
  ...
  async presentRange(options: DatePickerRangeOptions = {}): Promise<DatePickerRangeResult> {
    const normalized = normalizeOptions({ ...options, mode: 'date' });

    return new Promise<DatePickerRangeResult>((resolve) => {
      const start = createInput({ ...normalized, date: options.start ?? options.date, title: options.startTitle });
      const end = createInput({ ...normalized, date: options.end, title: options.endTitle });
      const overlay = this.createOverlay(normalized.title);
      const actions = createActions(normalized);
      const body = overlay.querySelector('[data-capgo-picker-body]');

      body?.append(wrapInput(options.startTitle ?? 'Start', start), wrapInput(options.endTitle ?? 'End', end));
      overlay.querySelector('[data-capgo-picker-actions]')?.append(actions.cancel, actions.done);

      const close = (result: DatePickerRangeResult): void => {
        this.closeActive();
        resolve(result);
      };
  ...
  private createOverlay(title?: string): HTMLDivElement {
    ...
    overlay.addEventListener('click', (event) => {
      if (event.target === overlay) {
        this.activePicker?.resolveEmpty();
      }
    });
  ...
  private openOverlay(overlay: HTMLDivElement, resolveEmpty: () => void): void {
    this.closeActive();
  ...
  private closeActive(): void {
    this.activePicker?.overlay.remove();
    this.activePicker = undefined;
  }
}

function normalizeOptions<T extends DatePickerOptions>(
  options: T,
): T & Required<Pick<DatePickerOptions, 'format' | 'mode'>> {
  const platform = options;
  const merged = {
    ...options,
    format: platform.format ?? DEFAULT_FORMAT,
    mode: platform.mode ?? 'dateAndTime',
    minuteStep: sanitizeStep(platform.minuteStep ?? platform.steps),
  };

  return merged as T & Required<Pick<DatePickerOptions, 'format' | 'mode'>>;
}

function createInput(options: DatePickerOptions & { mode: DatePickerMode; format: string }): PickerInput {
  const input = document.createElement('input') as PickerInput;
...
function createActions(options: DatePickerOptions): { cancel: HTMLButtonElement; done: HTMLButtonElement } {
  const cancel = createButton(options.cancelText ?? 'Cancel');
  const done = createButton(options.doneText ?? 'OK');
  return { cancel, done };
}
...
function wrapInput(label: string, input: HTMLInputElement): HTMLLabelElement {
...
function focusInput(input: PickerInput): void {
...
function inputTypeForMode(mode: DatePickerMode): string {
...
function inputValue(options: DatePickerOptions & { mode: DatePickerMode; format: string }): string {
  const date = parseDate(options.date, options.format, options.mode) ?? new Date();
...
function minMaxValue(value: string | undefined, mode: DatePickerMode, format: string): string {
  if (value === undefined) {
    return '';
  }
...
function stepValue(options: DatePickerOptions): string {
  const minuteStep = sanitizeStep(options.minuteStep ?? options.steps);
  if (minuteStep > 1) {
...
function dateFromInput(value: string, mode: DatePickerMode): Date | null {
  if (value.length === 0) {
    return null;
  }
  if (mode === 'time' || mode === 'countDownTimer') {
...
function parseDate(value: string | undefined, format: string, mode: DatePickerMode): Date | null {
...
function parseByFormat(value: string, format: string): Date | null {
  ...
  const parts: DateParts = { year: 1970, month: 1, day: 1, hour: 0, minute: 0, second: 0, millisecond: 0 };
  groups.forEach((group, index) => {
    const parsed = Number(match[index + 1]);
    if (group === 'yyyy') parts.year = parsed;
    if (group === 'MM') parts.month = parsed;
    if (group === 'dd') parts.day = parsed;
    if (group === 'HH') parts.hour = parsed;
    if (group === 'mm') parts.minute = parsed;
    if (group === 'ss') parts.second = parsed;
    if (group === 'SSS') parts.millisecond = parsed;
  });

  return new Date(parts.year, parts.month - 1, parts.day, parts.hour, parts.minute, parts.second, parts.millisecond);
}

function formatDate(date: Date, options: DatePickerOptions & { format: string }): string {
  const parts = partsFromDate(date, options.timezone);
...
function isDateOnly(value: string, mode: DatePickerMode): boolean {
  return mode === 'date' || /^\d{4}-\d{2}-\d{2}$/.test(value);
}

function emptyRange(): DatePickerRangeResult {
  return {
    start: null,
    end: null,
    value: null,
